...
- **`test_indicators.py`** — every indicator in `indicators.py` (`sma`, `ema`,
  `rsi`, `bollinger`, `obv`, `vwap`) checked against small, hand-worked inputs,
  plus the `Indicator` wrapper (registry lookup, parameter-key identity, result
  caching).
