...
XBRL_TAG_MAP = {
    ...
    # --- Financial Sector: Banks & Investment Banks ---
    # Net interest income: interest earned minus interest paid.
    # Primary revenue driver for commercial banks (JPM, BAC, WFC).
    'net_interest_income': [
        'InterestIncomeExpenseNet',       # JPM/GS: 170+ facts
        'InterestRevenueExpense',         # older tag variant
    ],
    # Non-interest income: fees, trading, advisory, commissions.
    # Second major revenue component for banks.
    'noninterest_income': [
        'NoninterestIncome',              # JPM/GS: 166+ facts
    ],
    # Non-interest expense: compensation, occupancy, tech, etc.
    # Primary expense line for banks (replaces operating_expenses).
    'noninterest_expense': [
        'NoninterestExpense',             # JPM/GS: 224+ facts
    ],
    # Provision for credit losses: expected loan losses.
    # Key risk indicator for banks.
    'provision_for_credit_losses': [
        'ProvisionForLoanLeaseAndOtherLosses',    # GS: 94 facts
        'ProvisionForLoanAndLeaseLosses',          # GS older: 58 facts
        'ProvisionForLoanLossesExpensed',           # BRK: 53 facts
        'ProvisionForCreditLosses',                 # alternate tag
    ],

    # --- Financial Sector: Insurance ---
    # Premiums earned: core revenue for insurers (BRK insurance ops).
    'premiums_earned': [
        'PremiumsEarnedNet',
        'PremiumsEarnedNetPropertyAndCasualty',   # BRK: 9 facts (deprecated but used)
    ],

    # --- Financial Sector: General ---
    # Investment income: income from investment portfolios.
    # Relevant for insurers, asset managers, and banks.
    'investment_income': [
        'InvestmentIncomeInterestAndDividend',    # BLK: 199 facts
        'InvestmentIncomeInterest',               # GS: 33 facts
        'NetInvestmentIncome',
        'GainLossOnInvestments',                  # BRK: 201 facts
    ],
}
...
class EdgarFundamentalsFetcher:
    ...
    DURATION_FIELDS = [
        ...
        # Financial sector duration fields
        'net_interest_income', 'noninterest_income',
        'noninterest_expense', 'provision_for_credit_losses',
        'premiums_earned', 'investment_income',
    ]
    ...
    def _compute_ratios(self, record):
        ...
        For financial-sector companies where 'revenue' is null,
        we use an effective revenue proxy for margin calculations:
          - Banks: net_interest_income + noninterest_income
          - Insurers: premiums_earned + investment_income
          - Otherwise: falls back to standard revenue
        """
        ratios = {}
        ...
        net_interest_income = record.get('net_interest_income')
        noninterest_income = record.get('noninterest_income')
        premiums_earned = record.get('premiums_earned')
        investment_income = record.get('investment_income')

        # Determine effective revenue for margin calculations.
        # For banks/investment banks: NII + non-interest income.
        # For insurers: premiums + investment income.
        # Standard companies: use revenue as-is.
        effective_revenue = revenue
        if not effective_revenue:
            if net_interest_income is not None or noninterest_income is not None:
                # Bank-style: sum whichever components are available
                effective_revenue = (net_interest_income or 0) + (noninterest_income or 0)
                if effective_revenue == 0:
                    effective_revenue = None
            elif premiums_earned is not None:
                effective_revenue = (premiums_earned or 0) + (investment_income or 0)
                if effective_revenue == 0:
                    effective_revenue = None
        ...
        # Operating margin
        if effective_revenue and operating_income and effective_revenue != 0:
            ratios['operating_margin'] = round(operating_income / effective_revenue, 4)
        else:
            ratios['operating_margin'] = None

        # Net margin
        if effective_revenue and net_income and effective_revenue != 0:
            ratios['net_margin'] = round(net_income / effective_revenue, 4)
        else:
    ...
    def _save_fundamentals_to_db(self, stock_id, records):
        ...
        for r in records:
            row = (
                ...
                # Financial sector
                r.get('net_interest_income'),
                r.get('noninterest_income'),
                r.get('noninterest_expense'),
                r.get('provision_for_credit_losses'),
                r.get('premiums_earned'),
                r.get('investment_income'),
                'edgar',
            )
            rows.append(row)
    ...
    with EdgarFundamentalsFetcher(db_config=DB_CONFIG) as fetcher:
        ...
        if records:
            ...
            for r in records[:3]:
                ...
                # Financial sector fields (populated for banks/insurers)
                if r.get('net_interest_income'):
                    print(f"    NII:      {r.get('net_interest_income'):>15,}")
                if r.get('noninterest_income'):
                    print(f"    NonII:    {r.get('noninterest_income'):>15,}")
                if r.get('provision_for_credit_losses'):
                    print(f"    PCL:      {r.get('provision_for_credit_losses'):>15,}")
                if r.get('premiums_earned'):
                    print(f"    Premiums: {r.get('premiums_earned'):>15,}")
                if r.get('investment_income'):
                    print(f"    InvInc:   {r.get('investment_income'):>15,}")
