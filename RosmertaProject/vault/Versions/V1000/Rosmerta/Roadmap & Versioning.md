...
- [x] Fundamentals download — *built (EDGAR)* ✅ 2026-07-23
- [ ] 10 years of price download — *built (IB hourly bars, chunked)*
- [ ] Basic DB helper scripts — *partial (`db_scripts/`, `db_tools/`)*
- [ ] 5 indicators — *built: `sma, ema, rsi, bollinger, obv, vwap` (6 technical);
  plus `days_offset_gain`, a **non-standard forward-looking calc** (added
  2026-07-03) that offsets each bar by `days_ahead × bars_per_day` bars and
  returns the pct/abs price change N days ahead — needs `source=['timestamp', <col>]`*
